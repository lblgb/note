...
当前阶段是 SQLite 数据驱动的最小笔记编辑闭环，并已接入账号注册、登录、退出、本地会话保存、登录后当前设备登记和手动同步。已支持文件夹列表、笔记列表、笔记详情切换，以及新建、编辑、保存和取消编辑。文件夹和笔记会保存到本地 SQLite，应用重启后仍可读取；点击顶部“同步”会把本地文件夹和笔记推送到服务端，再按本地游标增量拉取服务端状态写回本地。
...
启动后先注册或登录账号，认证成功后客户端会调用 `/api/devices/register` 登记当前设备，再进入本地笔记界面。点击顶部“同步”可调用 `/api/sync/push` 和 `/api/sync/pull` 验证文件夹和笔记同步；同步成功后状态会显示本次拉取到的文件夹和笔记数量。访问令牌过期时客户端会先刷新令牌并重试，刷新失败时会弹出“登录已失效”，可选择取消或重新登录。退出登录会清除本地 `auth_session.json` 并回到登录页。
